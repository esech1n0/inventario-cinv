...
// Función pura que modela la lógica transaccional de inventario
  ...
  describe("Salidas y Retiro de Material (OUT)", () => {
    ...
    it("debe descontar unidades y recalcular paquetes en artículos empaquetados", () => {
      const item = {
        ...
        packs: 10,
        unitsPerPack: 12,
        totalUnits: 120, // 10 paquetes de 12
      };

      // Retiramos 15 unidades (1 paquete completo y 3 sueltas)
      const result = processStockTransaction(item, {
        transactionType: "OUT",
        quantity: 15,
      });
      ...
      // 105 / 12 = 8 paquetes completos restantes
    ...
    it("REGLA CRÍTICA: NO debe permitir bajo ninguna circunstancia que el inventario quede negativo", () => {
      ...
      expect(() =>
        processStockTransaction(item, {
          transactionType: "OUT",
          quantity: 5, // Intento de retirar más de lo disponible
        })
