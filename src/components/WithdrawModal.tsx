...
interface WithdrawModalProps {
  ...
  onSuccessOptimistic: (itemId: string, quantity: number, motive: string, eventName?: string) => void;
...
export function WithdrawModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (quantity > currentItem.totalUnits) {
      setError(`Stock insuficiente. Solo hay ${currentItem.totalUnits} unidades disponibles.`);
      return;
    }
    ...
    // Disparar actualización optimista inmediata en la UI
    onSuccessOptimistic(currentItem.id, quantity, motive, isEventMotive ? eventName : undefined);
    onClose();
      ...
      <div className="animate-fade-in w-full max-w-lg rounded-t-3xl sm:rounded-2xl border border-border bg-card p-6 shadow-2xl">
        <div className="flex items-center justify-between pb-4 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-foreground">Retirar Material (Salida)</h2>
              <p className="text-xs text-muted-foreground">Registra el consumo o préstamo de inventario</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">Artículo a retirar</label>
            <select
              value={itemId}
              onChange={(e) => {
            ...
            {currentItem && (
              <p className="text-xs text-muted-foreground">
                Stock actual: <span className="font-semibold text-foreground">{currentItem.totalUnits} unidades</span>
                {currentItem.packagingType === "PACKAGED" && ` (${currentItem.packs} paquetes)`}
              </p>
            )}
          </div>
          ...
          {/* Cantidad a retirar */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">Cantidad de unidades a retirar</label>
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={() => setQuantity((q) => Math.min(currentItem?.totalUnits || 1, q + 1))}
                className="flex h-10 w-10 items-center justify-center rounded-xl border border-border bg-muted font-bold text-foreground hover:bg-accent"
              >
                +
              </button>
          ...
          {/* Motivo */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">Motivo del retiro *</label>
            <select
              value={motive}
          ...
          {/* Campo condicional para Nombre del Evento */}
          {isEventMotive && (
